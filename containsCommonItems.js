...
let map = {};
  for(let i=0; i< arr1.length; i++){
    if(!map[arr1[i]]){   //map[arr[i]] is false bs we rn we don't have any property in it. ! to make it true since if statements only run if they are trythy.
      const item = arr1[i]; // arr1[0] => const item = a
      map[item] = true;  // map[a] = true is added to the map obj.
      //This means we are storing the property in an object and it looks like this.
      //  {
      //     a: true      
      // }
    }
